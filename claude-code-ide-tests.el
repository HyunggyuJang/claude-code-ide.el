...
(defun claude-code-ide-tests--clear-processes ()
  "Clear workspace sessions and MCP sessions for testing.
Ensures a clean state before each test that involves session management."
  ;; Clear workspace sessions
  (when (boundp 'claude-code-ide--workspace-sessions)
...
(ert-deftest claude-code-ide-test-list-sessions-with-sessions ()
  ...
  (unwind-protect
      (progn
        ...
        ;; Manually add mock entries to the workspace sessions table
        (puthash "workspace1" `(:process ,(current-buffer) :directory "/tmp/project1" :buffer ,(current-buffer)) claude-code-ide--workspace-sessions)
        (puthash "workspace2" `(:process ,(current-buffer) :directory "/tmp/project2" :buffer ,(current-buffer)) claude-code-ide--workspace-sessions)

        ;; Verify we have 2 entries
        (should (= (hash-table-count claude-code-ide--workspace-sessions) 2))
  ...
  (unwind-protect
      (let ((claude-code-ide--cli-available t)
        ...
        (cl-letf (((symbol-function 'vterm)
          ...
          ;; Note: Buffer cleanup is no longer needed with pure MCP architecture
...
(ert-deftest claude-code-ide-test-mcp-server-lifecycle ()
  ...
  (require 'claude-code-ide-mcp)
  (let ((test-workspace "test-mcp-workspace")
        (claude-code-ide-mcp--sessions (make-hash-table :test 'equal))
        (test-port nil))
    (unwind-protect
        (progn
          ;; Start server for test workspace
          ...
          ;; Stop server for the test workspace
          (claude-code-ide-mcp-stop-session test-workspace)
          ;; Wait a moment for cleanup to complete
          (sleep-for 0.1)
          ;; Check lockfile removed
          (should-not (file-exists-p (claude-code-ide-mcp--lockfile-path test-port))))
      ;; Ensure cleanup - clean up by workspace name if session cleanup didn't work
      (when test-port
        (ignore-errors
          (claude-code-ide-mcp-stop-session test-workspace)
      ...
      (unwind-protect
          (progn
            ...
            (let ((default-directory project-a))
              (claude-code-ide-mcp-start "workspace-a")
              (setq session-a (gethash "workspace-a" claude-code-ide-mcp--sessions)))

            ;; Session B
            (let ((default-directory project-b))
              (claude-code-ide-mcp-start "workspace-b")
              (setq session-b (gethash "workspace-b" claude-code-ide-mcp--sessions)))
...
(ert-deftest claude-code-ide-test-websocket-integration ()
  ...
  (claude-code-ide-test-with-persp-mode-mocks "test-workspace"
    ...
    (unwind-protect
        (progn
          ;; Start MCP server
          (setq test-port (claude-code-ide-mcp-start))
          (should (numberp test-port))
          (setq test-server (claude-code-ide-mcp--get-current-session))
          (should test-server)
...
(ert-deftest test-claude-code-ide-serialize-session-state ()
  ...
  (let* ((test-directory "/test/directory")
    ...
    ;; Setup both workspace session and MCP session
    (puthash "test-workspace" test-session claude-code-ide--workspace-sessions)
    (puthash "test-workspace" mcp-session claude-code-ide-mcp--sessions)
    ...
    (unwind-protect
        (let ((serialized (claude-code-ide--serialize-session-state test-session)))
          ...
          ;; Note: buffer-name is no longer serialized with pure MCP architecture
...
;; Performance and stress testing suite (Pure MCP architecture)
...
(ert-deftest claude-code-ide-test-vterm-hook-integration ()
  "Test that vterm-mode advice correctly sets up environment variables for unmanaged terminals."
  (let* ((temp-dir (make-temp-file "test-vterm-" t))
         ...
         (test-called nil)
         (orig-vterm-mode (when (fboundp 'vterm-mode) (symbol-function 'vterm-mode))))
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'claude-code-ide--get-workspace-name)
                    ...
                    ;; Mock vterm-mode to avoid requiring vterm
                    ((symbol-function 'vterm-mode)
                     (lambda () (setq test-called t))))
            ...
            (let (captured-env)
              ...
              (should (member "TERM_PROGRAM=emacs" captured-env))
              (should (member "FORCE_CODE_TERMINAL=true" captured-env)))))
      ...
      (when (file-exists-p temp-dir)
        (delete-directory temp-dir t))
      ;; Restore original vterm-mode if it existed
      (when orig-vterm-mode
        (fset 'vterm-mode orig-vterm-mode)))))

(ert-deftest claude-code-ide-test-vterm-hook-managed-buffer-ignored ()
  "Test that managed vterm buffers (created by claude-code-ide) are ignored by the advice."
  (claude-code-ide-test-with-persp-mode-mocks "test-workspace"
          ...
          (progn
            ...
            (cl-letf (((symbol-function 'vterm-mode)
              ...
              (with-temp-buffer
                ...
                ;; Call vterm-mode which should trigger our advice
                (vterm-mode)
                (should test-called)

                ;; Environment should still be set even for managed buffers
                ;; (the advice doesn't distinguish buffer names, that's handled elsewhere)
...
(ert-deftest claude-code-ide-test-vterm-hook-no-workspace ()
  "Test that vterm advice gracefully handles cases with no workspace."
  (let ((original-env nil)
        (test-called nil)
        (orig-vterm-mode (when (fboundp 'vterm-mode) (symbol-function 'vterm-mode))))
